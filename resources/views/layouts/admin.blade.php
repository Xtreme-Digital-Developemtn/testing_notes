...
<body class="bg-gray-50 dark:bg-gray-950 text-gray-800 dark:text-gray-200 min-h-screen font-sans antialiased">
    ...
    <aside id="sidebar-mobile" class="fixed top-0 right-0 w-72 h-full bg-gray-900 text-white z-50 transform translate-x-full md:hidden overflow-y-auto">
        ...
        <nav class="p-3 space-y-1">
            @php
                $unreadAdminNotifications = \App\Models\AdminNotification::where('admin_id', auth()->guard('admin')->id())->where('is_read', false)->count();
                $navItems = [
                    ['route' => 'admin.dashboard', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-4 0a1 1 0 01-1-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 01-1 1" />', 'label' => 'لوحة التحكم', 'badge' => 0],
                    ['route' => 'admin.requests.index', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />', 'label' => 'الطلبات', 'badge' => 0],
            ...
            @foreach($navItems as $item)
                ...
                <a href="{{ route($item['route']) }}" class="sidebar-link flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium {{ $active ? 'active text-white' : 'text-white/70' }}">
                    ...
                    {{ $item['label'] }}
                    @if($item['badge'] > 0)
                        <span class="mr-auto bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{{ $item['badge'] }}</span>
                    @endif
                </a>
            @endforeach
        </nav>
        ...
        <aside class="hidden md:flex w-60 lg:w-64 shrink-0 bg-gray-900 dark:bg-gray-950 text-white flex-col border-l border-gray-800 sticky top-0 h-screen overflow-y-auto">
            ...
            <div class="p-4 border-t border-white/10">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-admin-500/80 flex items-center justify-center font-bold text-sm">A</div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium truncate">{{ auth()->guard('admin')->user()->name }}</p>
                        <p class="text-xs text-white/50 truncate">{{ auth()->guard('admin')->user()->email }}</p>
                    </div>
                </div>
            </div>
        </aside>
