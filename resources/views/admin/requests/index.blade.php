...
@section('content')
    <h1 class="text-2xl font-bold">جميع الطلبات</h1>
    ...
    <form method="GET" class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <input type="text" name="search" placeholder="بحث..." value="{{ request('search') }}" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary focus:border-transparent">
            ...
            <select name="admin_id" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2.5 text-sm">
                <option value="">كل من حل الطلبات</option>
            ...
            <div class="flex gap-2">
                <button type="submit" class="flex-1 bg-gray-800 dark:bg-gray-700 text-white px-4 py-2.5 rounded-lg text-sm">بحث</button>
                <a href="{{ route('admin.requests.index') }}" class="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 py-2.5 rounded-lg text-sm text-center">إعادة</a>
            </div>
        </div>
    </form>
                ...
                <table class="w-full text-right text-sm">
                    <thead>
                        <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                            <th class="py-3 px-5 text-gray-500 dark:text-gray-400 font-medium">العميل</th>
                            ...
                            <th class="py-3 px-5 text-gray-500 dark:text-gray-400 font-medium">حل بواسطة</th>
                    ...
                    <tbody>
                        @foreach($requests as $req)
                            <tr class="border-b border-gray-100 dark:border-gray-800 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="py-3.5 px-5">{{ $req->user->name }}</td>
                                <td class="py-3.5 px-5">
                                    <a href="{{ route('admin.requests.show', $req->id) }}" class="text-primary hover:underline font-medium">{{ $req->title }}</a>
                                </td>
