    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                <span class="text-gray-500 dark:text-gray-400 text-xs">العميل</span>
                <p class="mt-1.5 text-sm font-medium">{{ $clientRequest->user->name }}</p>
            </div>
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
            ...
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                <span class="text-gray-500 dark:text-gray-400 text-xs">التاريخ</span>
        ...
        @if($clientRequest->image_path)
            ...
            <div class="mb-6">
                ...
                <a href="{{ route('storage.serve', $imageFile?->id) }}" class="inline-flex items-center gap-2 mt-2 bg-admin-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-admin-700 transition-colors">
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
            ...
            <form method="POST" action="{{ route('admin.requests.updateStatus', $clientRequest->id) }}" class="flex flex-wrap items-center gap-4">
                ...
                <select name="status" class="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2.5 text-sm">
                    <option value="not_started" {{ $clientRequest->status == 'not_started' ? 'selected' : '' }}>لم تبدأ</option>
                    <option value="in_progress" {{ $clientRequest->status == 'in_progress' ? 'selected' : '' }}>قيد التنفيذ</option>
                    <option value="solved" {{ $clientRequest->status == 'solved' ? 'selected' : '' }}>تم الحل</option>
                </select>
                <button type="submit" class="bg-admin-500 text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-admin-700 transition-colors">تحديث الحالة</button>
            </form>
        </div>

        <div class="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
            <h2 class="text-lg font-bold mb-4">إرسال رسالة</h2>
            <a href="{{ route('admin.messages.create', ['user_id' => $clientRequest->user_id, 'request_id' => $clientRequest->id]) }}" class="inline-flex items-center gap-2 bg-green-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-green-700 transition-colors">
